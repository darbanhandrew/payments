...
class StripeSettings(Document):
    ...
    def on_update(self):
        ...
        call_hook_method("payment_gateway_enabled",
                         gateway="Stripe-" + self.gateway_name)
    ...
    def validate_stripe_credentails(self):
        if self.publishable_key and self.secret_key:
            ...
            try:
                ...
                frappe.throw(
                    _("Seems Publishable Key or Secret Key is wrong !!!"))
            ...
            if flt(amount) < self.currency_wise_minimum_charge_amount.get(currency, 0.0):
                frappe.throw(
                    _("For currency {0}, the minimum transaction amount should be {1}").format(
                        currency, self.currency_wise_minimum_charge_amount.get(
                            currency, 0.0)
                    )
                )
    ...
    def create_charge_on_stripe(self):
        ...
        try:
            ...
            if charge.captured == True:
                ...
                frappe.log_error(charge.failure_message,
